...
# Import von Python Standarard Bibliotheken (müssen trozdem zum nutzen importiet werden)
import sys                                                                  # Zugriff auf Interpreter/Programm-Umgebung (z.B. um das Programm zu beenden)
import os                                                                   # Zugriff auf Betriebssystem-Funktionen (Ordener anlegen, Pfade verwalten, Dateimanagement)  
import struct                                                               # Übersetz Hexbytes in Datentypen ( z.B. float) und zurück (Essenziell für Kommunikation mit Spektrometer die in Hexbytes --> siehe Doc)
import time                                                                 # Zählt seit Januar 1970 (Unix Epoch) die Sekunden (z.B. für Zeitdifferenzmessung, Zeit in sek warten..)
import csv                                                                  # Um CSV-Dateien schreiben/lesen zu können
import threading                                                            # Nebenläufigkeit (Threads), damit mehrere Aufgaben parallel laufen (z.B. Gleichzitigkeit der Aufnahme von Kamera und Spektrometer)
from datetime import datetime                                               # Zum Auslesen der Systemzeit die über die angebrachte RTC (Reakl Time Clock) vorgegeben wird

# Import von externen Python Bibliotheken (müssen zuerst installiert werden z.B. mittels pip install) 
# Daher auch Prüfung ob die Module installiert sind 
try:
    import serial                                                           # Zum Lesen und Schreiben von bytes über Serielle Schnittstelle 
...
## Definition der Klasse (SpektrometerApp) in der das eigentliche Programm mit seinen Eigenschaften (self) und seinen Funktionen liegen 
...
## Programmaufruf (Vorher nur definition der Funktnionen und Eigenschaften der Klasse: SpektrometerApp)
 
if __name__ == "__main__":                  # Die Funktionsaufrufe hierunter werden nur ausgeführt wenn das Programm selber aufgerufen wird aber nicht wenn es als Modul importiert wird 
                                            # Variable __name__ wird zu Beginn eines Python Skripts immer automatisch besetzt (vergibt bei import: __Modulname___; und bei Ausführung: __main__)
                                            # Verhindert das dieses Programm beim import in einem anderen Programm als Modul direkt kompiliert wird
    root = tk.Tk()                          #
